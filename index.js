...
if (!process.env.BOT_TOKEN) {
    console.error('Error: BOT_TOKEN is not defined. Please add it to your .env file.');
    process.exit(1);
}
    ...
    if (req.session.user) {
        ...
        db.get(sql, [userId], (err, row) => {
            if (err) {
                ...
                return res.status(500).json({ success: false, message: 'Error getting your points.' });
            }
            const totalPoints = row ? row.total_points : 0;
            res.json({ loggedIn: true, user: { ...req.session.user, total_points: totalPoints } });
        });
    } else {
...
app.post('/tg-login-start', (req, res) => {
    ...
    db.run('INSERT INTO tokens (token, status) VALUES (?, ?)', [token, 'pending'], (err) => {
        if (err) {
            console.error('Failed to create token', err);
            return res.status(500).json({ success: false, message: 'Could not start login process.' });
        }
        res.json({ token });
    });
});
...
app.post('/tg-login-check', (req, res) => {
    ...
    db.get('SELECT * FROM tokens WHERE token = ?', [token], (err, tokenData) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: 'Error checking login status.' });
        }
...
app.post('/spin', (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ success: false, message: 'You must be logged in to spin the wheel.' });
    }
    ...
    db.get(sql, [userId, today], (err, row) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: 'Error checking your spin history.' });
        }
        ...
        db.run(insertSql, [userId, pointsWon], (insertErr) => {
            if (insertErr) {
                ...
                return res.status(500).json({ success: false, message: 'Error saving your spin.' });
            }
...
app.post('/withdraw', (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ success: false, message: 'You must be logged in to make a withdrawal request.' });
    }
    ...
    db.get(sql, [userId], (err, row) => {
        ...
        if (totalPoints < points) {
            return res.status(400).json({ success: false, message: 'You do not have enough points to make this withdrawal.' });
        }
        ...
        db.run(insertSql, [userId, points, solanaAddress], (insertErr) => {
            if (insertErr) {
                ...
                return res.status(500).json({ success: false, message: 'Error creating your withdrawal request.' });
            }
            ...
            db.run(deductSql, [userId, -points], (deductErr) => {
                if (deductErr) {
                    ...
                    return res.status(500).json({ success: false, message: 'Error updating your points.' });
                }
...
app.get('/withdraw-history', (req, res) => {
    if (!req.session.user) {
        return res.status(401).json({ success: false, message: 'You must be logged in to view your withdrawal history.' });
    }
    ...
    db.all(sql, [userId], (err, rows) => {
        if (err) {
            ...
            return res.status(500).json({ success: false, message: 'Error getting your withdrawal history.' });
        }
...
bot.start((ctx) => {
    ...
    if (!token) {
        return ctx.reply('Welcome! Please start the login process from our website.');
    }
    ...
    db.get('SELECT * FROM tokens WHERE token = ?', [token], (err, tokenData) => {
        ...
        if (tokenData.status === 'pending') {
            ...
            `;
            db.run(userSql, [user.id, user.username, user.first_name, user.last_name], (userErr) => {
                if (userErr) {
